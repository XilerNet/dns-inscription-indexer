...
pub(crate) async fn save_inscription(
  ...
  let db = get_db().await;
  let content_type = inscription.content_type().unwrap_or("unknown");

  if !content_type.starts_with("text/plain") {
    return;
  }

  let content = inscription.body().unwrap_or(&[]);
...
pub(crate) async fn new_inscription(
  ...
  if let Some(inscription) = inscription {
    return save_inscription(id, inscription, address).await;
  }
...
pub(crate) fn new_inscription_blocking(
  ...
  rt.block_on(new_inscription(id, inscription, address));
}
